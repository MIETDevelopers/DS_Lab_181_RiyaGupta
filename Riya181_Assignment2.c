...
int main()
{
    ...
    sleep(1);
    t = clock() - t;
    double time_taken_by_bubble_sorting = ((double)t)/CLOCKS_PER_SEC; // in milliseconds
  
    printf("bubble sorting took %f ms to execute \n", time_taken_by_bubble_sorting);
    printf("\n\n--------------------------------\n\n");
    t = clock();
    ...
    printf("merge sorting took %f ms to execute \n", time_taken_by_merge_sorting);
    return 0;
}
